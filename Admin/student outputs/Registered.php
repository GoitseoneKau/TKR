	  ...
	  if($conn){
		...
		$output.="
				...
				<div class='text-right'>
                      <button type='button' class='btn btn-secondary' data-toggle='modal' data-target='#myModal'>Register New Student</button>
                  </div>
                  <br/>
					  ...
					  $output.="<tr>
								...
								<div class='modal-header'>
										...
										<div class='modal-body'>
											<div class='container-fluid'>
												<div class='row'>
													<div class='col-md-12'><form class='form-center'>
														";
														...
														for ($i=0;$i<count(getMatricKeys());$i++){
															...
															if(!preg_match("/(jpeg|jpg|png)/", strtolower( pathinfo($row[getMatricKeys()[$i]],PATHINFO_EXTENSION) ))){
																  ...
																  $output.="<div class='form-group'>
																  <label class='form-control-label'>".getMatricKeys()[$i].": </label>
																  <img class='img-thumbnail h-50 w-100' id='' src='../".$row[getMatricKeys()[$i]]."' width='250px' height='500px'/>
																  </div>";
															}
													 	}
						...
						$output.=							"</form></div>
										...
										<div class='modal-footer'>
											...
											<button type='button'  onclick='UpdateSubmit(".$row['matric_id'].")'  class='btn btn-primary'>submit</button>
										</div>
									</div>
								</div>
				...
				while($row = mysqli_fetch_assoc($result)){
					  $output.="<tr>
													...
													<div class='col-md-12'><form class='form-center'>
														...
														for ($i=0;$i<count(getSkillKeys());$i++){
															...
															if(!preg_match("/(jpeg|jpg|png)/", strtolower( pathinfo($row[getSkillKeys()[$i]],PATHINFO_EXTENSION) ))){
																  ...
																  <input type='text' class='form-control' id='' name='".getSkillKeys()[$i]."' value='".$row[getSkillKeys()[$i]]."'>
																  </div>";
															}else{
																  $output.="<div class='form-group'>
																  <label class='form-control-label'>".getSkillKeys()[$i].": </label>
																  <img class='img-thumbnail h-50 w-100' id='' src='../".$row[getSkillKeys()[$i]]."' width='250px' height='250px/>
																  </div>";
															}
													  }
						...
						$output.=							"</form></div>
										...
										<div class='modal-footer'>
											<button type='button' class='btn btn-secondary' data-dismiss='modal'>Close</button>
											<button type='button' onclick='UpdateSubmit(".$row['student_id'].")'   class='btn btn-primary'>submit</button>
										</div>
									</div>
								</div>
		...
		if($request=="ac"){
			if(!empty($_POST['request_header']) && !empty($_POST['request_value']) ){
				$request_h = $_POST['request_header'];
				$request_d = $_POST['request_value'];
				$sql="SELECT * FROM tkr_afternoon_class WHERE ".$request_h." LIKE '".$request_d."%' OR ".$request_h."=".$request_d;
			}else{
			...
			$output .= "<div class='table-responsive'>
                                    <table class='table table-bordered table-hover table-striped' id='table-data'>
                                        <thead>
										<tr class='bg-danger'><th colspan='100%' class='text-center'>".$title."  ".$sql."</th></tr>
				...
				while($row = mysqli_fetch_assoc($result)){
					  $output.="<tr>
													...
													<div class='col-md-12'><form class='form-center'>
														...
														for ($i=0;$i<count(getAfternoonKeys());$i++){
															  ...
															  if(!preg_match("/(jpeg|jpg|png)/", strtolower( pathinfo($row[getAfternoonKeys()[$i]],PATHINFO_EXTENSION) ))){
																	...
																	$output.="<div class='form-group'>
																	<label class='form-control-label'>".getAfternoonKeys()[$i].": </label>
																	<img class='img-thumbnail h-50 w-100 id='' src='../".$row[getAfternoonKeys()[$i]]."' width='250px' height='250px/>
																	</div>";
															  }
														}
						...
						$output.=							"</form></div>
										...
										<div class='modal-footer'>
											<button type='button' class='btn btn-secondary' data-dismiss='modal'>Close</button>
											<button type='button'  onclick='UpdateSubmit(".$row['class_id'].",'form".$row['class_id']."')'  class='btn btn-primary'>submit</button>
										</div>
									</div>
								</div>
