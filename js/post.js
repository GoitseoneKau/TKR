$(document).ready(function(){
            ...
            $("form#personalDet").submit(function(event){
					...
					if($("input[name=inlineRadioBOptions]:checked").val() =="no"){
						...
						$.ajax({
							...
							success: function(data){
								...
								if(data.status==true){
									$(".nextOfKinDet").css("display","block");
									$(".personalDet").css("display","none");	
								}
				...
				function(event){
					...
					$.ajax({
						...
						success: function(data){
							...
							//code to go to next page
							if(data.status==true){
								$(".parentGuardianDet").css("display","block");
								$(".nextOfKinDet").css("display","none");
							}
					...
					$.ajax({
						...
						success: function(data){
							...
							if(data.status==true){
								...
								$(".parentGuardianDet").css("display","none");
							}
			...
			$("form#declaration").submit(
              ...
              let select1;
              select1 += '<option val ="0" disabled> Please select your age </option>';
              for( let i = 8; i <=35 ; i++){
